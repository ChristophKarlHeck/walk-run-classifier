// In app/src/androidTest/java/com/example/walkrunclassifier/testutils/CsvAccelerometerDataSource.kt
// (or your preferred test package)
 ...
 * @param csvFileName The name of the CSV file in the "assets" directory (e.g., "walking_data.csv").
 * @param emissionDelayMillis The delay in milliseconds between emitting each data point.
 *                            This simulates a sensor sampling interval. Default is 20ms (~50Hz).
...
class CsvAccelerometerDataSource(
    ...
    private val emissionDelayMillis: Long = 20L, // Approx 50Hz
    ...
    override fun getAccelerometerData(): Flow<AccelerometerData> = callbackFlow {
        ...
        val job = launch(Dispatchers.IO) { // Perform file reading on IO dispatcher
            try {
                // Accessing assets from src/androidTest/assets
                testContext.assets.open(csvFileName).use { inputStream ->
                    BufferedReader(InputStreamReader(inputStream)).use { reader ->
                        ...
                        while (isActive && line != null) {
                            val currentLine = line!! // Smart cast, or use line!! if needed by flow
                            ...
                            if (parts.size >= 3) {
                                ...
                                if (x != null && y != null && z != null) {
                                    ...
                                    // Log.v(TAG, "Emitting from $csvFileName: $data") // Verbose logging
                                    trySend(data).isSuccess // isSuccess can be checked for backpressure handling
                                } else {
                                    Log.w(TAG, "Skipping malformed line in $csvFileName (line $lineNumber, null after parsing): '$currentLine'")
                                }
                            } else {
                                Log.w(TAG, "Skipping line in $csvFileName (line $lineNumber, not enough parts, expected 3, got ${parts.size}): '$currentLine'")
                            }
                        ...
                        Log.i(TAG, "Finished reading all lines from $csvFileName. Total lines processed: ${lineNumber -1}.")
                    }
                }
            } catch (e: IOException) {
                ...
                if (!isClosedForSend) {
                    Log.d(TAG, "Closing flow for $csvFileName as reading is complete or an error occurred.")
